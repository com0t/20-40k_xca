							...
							foreach ( $category as $option => $value ) {
								if ( $category_name === 'areas' && $option === 'parse_css_files' ) {
                                    if($value === true  && $current_options->areas->parse_css_files === false) {
                                        ...
                                        if(AccessControlHeaders::addHeadersToHtaccess() < 0) {
                                            $success = false;
                                            $message = __( 'Options were saved but could not add the CORS header to .htaccess.', 'shortpixel-adaptive-images' ) . ' '
                                                . sprintf(__( 'Please verify that the <strong>%s.htaccess</strong> file is writable.', 'shortpixel-adaptive-images' ), get_home_path());
                                        }
                                    } else if($value === false && $current_options->areas->parse_css_files === true) {
                                        if(AccessControlHeaders::removeHeadersFromHtaccess() < 0) {
                                            $success = false;
                                            $message = __( 'Options were saved but could not remove the CORS header from .htaccess.', 'shortpixel-adaptive-images' ) . ' '
                                                . sprintf(__( 'Please verify that the <strong>%s.htaccess</strong> file is writable.', 'shortpixel-adaptive-images' ), get_home_path());
                                        }
                                    }
                                }
